...
// TestMergeAndSortRulesKeepsAllOutboundParams pins the rule-merge identity
// site: the merge decision compared Outbound.String, whose display form
// ellipsizes params from index 5 on, so two rules whose outbounds differ only
// in a late param were treated as the same outbound and merged. (Port of kdae
// a4cffe92, P3-3.)
func TestMergeAndSortRulesKeepsAllOutboundParams(t *testing.T) {
	...
	if len(merged) != 1 {
		t.Fatalf("identical outbounds no longer merge: got %d rules, want 1", len(merged))
	}
}
